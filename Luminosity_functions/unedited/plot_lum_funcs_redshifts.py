...
idx1 = [0,0,1,1,2,2]
idx2 = [0,1,0,1,0,1]
bin_size = [0.8,0.4,0.4,0.5,0.6,0.2]
bin_size_PS = [1.,1.4,0.6,0.8,1.,0.8]
...
#Kondapally 2022
LF_Konda_05_1 = np.loadtxt('LFs_Rohit/AGN_LF_Kondapally22/radio-excess_AGN/LF_radio_excess_AGN_0.7_z_1.0_tab.csv', delimiter=',', skiprows = 1, usecols = (0,1,2,3))
LF_Konda_1_15 = np.loadtxt('LFs_Rohit/AGN_LF_Kondapally22/radio-excess_AGN/LF_radio_excess_AGN_1.3_z_1.7_tab.csv', delimiter=',', skiprows = 1, usecols = (0,1,2,3))
LF_Konda_15_2 = np.loadtxt('LFs_Rohit/AGN_LF_Kondapally22/radio-excess_AGN/LF_radio_excess_AGN_1.7_z_2.1_tab.csv', delimiter=',', skiprows = 1, usecols = (0,1,2,3))
# LF_Konda_2_25 = np.loadtxt('LFs_Rohit/AGN_LF_Kondapally22/LERG/LF_LERG_AGN_2_z_2.5_tab_quies.csv', delimiter=',', skiprows = 1, usecols = (0,1,2,3))
...
# outer = gridspec.GridSpec(3,2, wspace = 0, hspace = 0)

for index, z_bin_min in enumerate(redshift_bins[:-2]):
    for LF_name in LF_type:
        hdulist = fits.open('data/lum_func_weighted_'+LF_name+'.fits')
        ...
        if LF_name == 'SDSS':
            ...
            marker_face, marker_face_PS = 'none', 'none'
            label_name = 'photo-z'
            m_i = '22.5'
        ...
        # define PS sample
        ind_peaked = np.where((alpha_low_z >= 0.1) & (alpha_high_z <= 0.0))
        ...
        ax.errorbar(central_bins_144, log_lum_func_144, xerr=np.diff(bin_edges_144)/2, yerr = log_err_lum_func_144, c = 'k', \
                    marker = 'o', markersize = 10, linestyle ='none', mfc = marker_face, capthick = 2, label = 'Master Sample\n'+label_name+' selected, $m_i\, <$'+ m_i)
        ax.errorbar(central_bins_144_PS, log_lum_func_144_PS, xerr=np.diff(bin_edges_144_PS)/2, yerr = log_err_lum_func_144_PS, c = 'crimson',\
                    marker = '^',  capthick = 2, markersize = 10, mfc = marker_face_PS, linestyle = 'none', label = 'PS sample\n'+label_name+' selected, $m_i\, <$'+ m_i)
        ...
        if index != 5:
            interp_LF = np.interp(central_bins_144_PS, central_bins_144, log_lum_func_144)
            resid = interp_LF - log_lum_func_144_PS
            
            i_min_diff = []
            for x_PS in (central_bins_144_PS):
                i_min_diff.append(np.argmin(abs(x_PS - central_bins_144)))
                
            log_err_x = log_err_lum_func_144[:,i_min_diff]
            err_resid = np.sqrt(log_err_lum_func_144_PS**2 + log_err_x**2)
            ...
            if LF_name == 'SDSS':
                ...
                ax1.errorbar(central_bins_144_PS, resid, yerr = err_resid, linestyle = ':', marker = markers[index],\
            ...
            ax1.set_xlim(22.5,29.5)
            ax1.set_ylim(-0.15,1.9)
            ax1.legend()
    
fig.savefig('plots/LF_SDSS.png')
fig2.savefig('plots/LF_resid_V_i.png')
