...
for i_bins in range(1): #range(len(bins_list)):, change range to plot for all brightness cuts or just the 0 mJy cut
    ...
    if sample=="VLASS":
        ind_peaked = np.where((alpha_low >= 0.0) & (alpha_high <= 0.0))      
    else:
    ...
    plt.savefig('/net/vdesk/data2/bach1/ballieux/master_project_1/colour_colour_plots/colour_colour_'+sample+'_mega.png')
    # plt.show()
