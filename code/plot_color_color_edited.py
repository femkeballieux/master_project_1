...
def hist_norm_height(n,bins,const):
    ...
    return n,new_bins[:-1]
def color_color(filename, main=1, overplot=2, outputname='test'):
    #read in data
    hdulist = fits.open('/net/vdesk/data2/bach1/ballieux/master_project_1/data/'+filename)
    ...
    alpha_low_main = tbdata['alpha_low_'+str(main)]
    alpha_high_main = tbdata['alpha_high_'+str(main)]
    flux_LoTSS_main = tbdata['LoTSS_flux_'+str(main)]
    ...
    for i_bins in range(1): #range(len(bins_list)):, change range to plot for all brightness cuts or just the 0 mJy cut
        ...
        source_bright_ind = np.where((flux_LoTSS_main > brightness_limit))
    
        alpha_low_main = alpha_low_main[source_bright_ind]
        alpha_high_main = alpha_high_main[source_bright_ind]
    
        err_alpha_low_main = np.median(tbdata['e_alpha_low_'+str(main)][source_bright_ind])
        err_alpha_high_main = np.median(tbdata['e_alpha_high_'+str(main)][source_bright_ind])
        print(err_alpha_low_main, err_alpha_high_main)
        ...
        try:
            H, X, Y = np.histogram2d(alpha_low_main.flatten(), alpha_high_main.flatten(), bins=(X, Y))
        ...
        X, Y = X[:-1], Y[:-1]
        ax.plot(alpha_low_main, alpha_high_main, "o", color='k', ms=2, zorder=-1,alpha=0.2,rasterized=True)
        
        #Below I make a mask for all the PS in overplot, and plot them as main
        alpha_low_overplot = tbdata['alpha_low_'+str(overplot)][source_bright_ind]
        alpha_high_overplot = tbdata['alpha_high_'+str(overplot)][source_bright_ind]
        
        overplot_mask = (alpha_low_overplot >= 0.1) & (alpha_high_overplot<=0)
        ax.plot(alpha_low_main[overplot_mask], alpha_high_main[overplot_mask], "o", color='red', ms=2, zorder=10,alpha=1,rasterized=True)      
        ...
        ax.set_title(outputname, fontsize = 25, loc='right')
        ...
        (_, caps, _) = ax.errorbar(-2.15,.5, xerr=err_alpha_low_main, yerr=err_alpha_high_main, color = 'k', linestyle='none',elinewidth=2)
        for cap in caps:
            cap.set_color('k') 
            cap.set_markeredgewidth(2)
    
    
        ind_peaked = np.where((alpha_low_main >= 0.1) & (alpha_high_main <= 0.0))
        ...
        n_high, bins_high = np.histogram(alpha_high_main.flatten(), bins=Y)
        ...
        ax1.plot(np.linspace(0,1,50),np.ones(50)*np.median(alpha_high_main),'k--')
        ...
        n_low, bins_low = np.histogram(alpha_low_main.flatten(), bins=X)
        ...
        ax2.plot(np.ones(50)*np.median(alpha_low_main), np.linspace(0,1,50), 'k--')
        ...
        plt.savefig('/net/vdesk/data2/bach1/ballieux/master_project_1/colour_colour_plots/colour_colour_'+ outputname + str(brightness_limit) + '.png')
        # plt.show()
    
        a_low_siqr = iqr(alpha_low_main)/2
        a_high_siqr = iqr(alpha_high_main)/2
        ...
        print('a_low median + std', np.median(alpha_low_main), np.std(alpha_low_main), a_low_siqr)
        print('a_high median + std', np.median(alpha_high_main[ind_peaked]), np.std(alpha_high_main[ind_peaked]), a_high_siqr)
#color_color('crossmatch_old_new_master.fits',2,1, 'main_Martje')
color_color('crossmatch_old_new_master.fits',1,2, 'main_mine')