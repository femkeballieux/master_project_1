...
source_bright_ind = np.where((flux_sample > 0.))
...
err_alpha_low = np.median(tbdata2['new_e_alpha_low_HF'])
...
if sample == "VLASS":
    ...
    ind_peaked = np.where((alpha_low > (tbdata2['new_e_alpha_low_HF']))
                          & (alpha_high < 0.))
...
ax.plot(alpha_low[ind_peaked], alpha_high[ind_peaked], "o", color='red',
        ms=3, zorder=-1, alpha=0.2, rasterized=True)
...
plt.savefig(
    '/net/vdesk/data2/bach1/ballieux/master_project_1/colour_colour_plots/official_colour_colour_'+sample+'_new_e_alpha_low', bbox_inches='tight', dpi=500)
# plt.show()
...
print('outliers>1.5', len(alpha_high[alpha_high >1.5]))
print('Source with alpha_low > 2.5:', len(alpha_low[alpha_low > 2.5]), 'with coordinates', tbdata['RA'][source_bright_ind][alpha_low > 2.5],
  tbdata['DEC'][source_bright_ind][alpha_low>2.5], 'and alpha_low', alpha_low[alpha_low>2.5] , \
      '+/-' , tbdata['e_alpha_low_'+sample][source_bright_ind][alpha_low>2.5], 'and alpha_high', alpha_high[alpha_low>2.5] , \
          '+/-' , tbdata['e_alpha_high_'+sample][source_bright_ind][alpha_low>2.5], tbdata['LoTSS_name'][source_bright_ind][alpha_low > 2.5] )
...
# print("95% distribution of alpha_low", np.median(alpha_low)-limit_95)
    #For HF
limit_95 = iqr(alpha_low, rng=(5,50)) 
print("95% distribution of alpha_low", np.median(alpha_low) - limit_95, 'USE this for scaling LF sample')
print('')
print(np.mean(tbdata['LoLSS_flux'][source_bright_ind]), np.mean(tbdata['LoTSS_flux'][source_bright_ind]),\
      np.mean(tbdata['NVSS_flux'][source_bright_ind]),  np.mean(tbdata['VLASS_flux'][source_bright_ind]))
print(np.median(tbdata['e_alpha_low_'+sample][source_bright_ind]), 'USE THIS FOR SCALING MPS OR GPS SAMPLE')